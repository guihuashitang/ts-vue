...
export default {
  require: [{ required: true, message: '该项不能为空', trigger: 'blur'}],
  changerequire: [{ required: true, message: '该项不能为空', trigger: 'change'}],
  phone: [{ required: true, pattern: /^1\d{10}$/, message: '目前只支持中国大陆的手机号码', trigger: 'blur' }],
  numPot2: [{ required: true, validator: checkNumPot2, trigger: 'blur' }],
  InterNum: [{ required: true, validator: checkInterNum, trigger: 'blur' }],
  region: [{ required: true, message: '选项不能为空', trigger: 'change' }],
}
